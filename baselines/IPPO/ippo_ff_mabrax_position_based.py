...
def make_train(config, rng_init):
    ...
    env = LogWrapper(env, replace_info=True) # changed this to false to log tracked metrics like success
    ...
    def train(rng):        
        ...
        def _update_step(runner_state, unused):
            ...
            def _env_step(runner_state, unused):
                ...
                info.update({"success": jnp.repeat(env_state.env_state.info["success"][:,None], env._env.num_agents, axis=1)}) #added line to track success metric
                info.update({"curr_success": jnp.repeat(env_state.env_state.info["curr_success"][:,None], env._env.num_agents, axis=1)}) #added line to track success metric
            ...
            def callback(metric):
                metric = {k: v.item() if isinstance(v, (jnp.ndarray, np.ndarray)) and v.size == 1 else v 
            ...
            print("traj batch info", traj_batch.info)
