...
class AntSoccerV2(PipelineEnv):
  ...
  def __init__(
        ...
        reset_noise_scale=0.1, #0.1
        exclude_current_positions_from_observation=False,
        backend="generalized",
        dense_reward: bool = False, #False
        **kwargs,
  ):
    path = epath.resource_path('brax') / 'envs/assets/ant_soccer_v2.xml'
    sys = mjcf.load(path)
    ...
    self.state_dim = 31
    self.goal_indices = jp.array([29, 30])
  ...
  def reset(self, rng: jax.Array) -> State:
    ...
    dist = 20 # originally 5, and then 10 in the other mabrax
